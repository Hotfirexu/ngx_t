...
void ngx_master_process_cycle() {
    ...
    for (;;) {
        ...
        /*
            sigsuspend();
            a)、根据给定的参数设置新的mask，并阻塞当前进程（因为是一个空集，所以不阻塞任何信号）
            b)、此时，一旦收到信号，便恢复原先的信号屏蔽【上面的mask设置了多达10个信号，从而保证下边的执行流程不会再次被其他信号截断】
            c)、调用该信号对应的信号处理函数
            d)、信号处理函数返回后，sigsuspend返回，使程序继续往下走
        */
...
static int ngx_spawn_process(int inum,const char *pprocname) {
    ...
    case 0:             //子进程分支
        ...
        ngx_worker_process_cycle(inum,pprocname);       //希望所有worker子进程，在这个函数里不断循环着不出来，也就是说，子进程流程不往下边走;
        break;
